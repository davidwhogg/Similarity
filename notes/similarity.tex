...
\quad ,
\end{eqnarray}
where $Z$ is a normalization constant that ensures a unit integral in
the $b$ space. Note that this solution has the very nice property that
if $R$ is not square---that is, the prior variance is low rank---all
computations are done in the lower-dimension basis, with the operator
$W\cdot\transpose{R}\cdot\inverse{C}$ projecting the data difference
$[y_1-y_2]$ to the lower-dimension basis.

In terms of generating a scalar: One option is the \emph{value} of
this log probability $\ln p(b\given y_1,y_2)$ evaluated at $b=0$. That
has lots of good properties, including that of being probabilistically
justified, and that of not depending incredibly strongly on the prior
variance $\Lambda$ (especially when the data are high in
signal-to-noise). Another good (and surprising) property is that it
becomes exactly $\chi^2$ when the measurements are better than the
prior variance along all directions, and the prior variance is
full-rank.
