...
% # To-Do:
% - see orange book.
% - put in proper bibliography and citations.
% - link to relevant Wikipedia pages.
...
\footnote{This document is copyright 2017 the author. Feel free to copy and
distribute it, provided that you make no changes to it whatsoever.
For many of the ideas and some of the presentation in this document, I
...
\end{eqnarray}
where $R$ is rectangular, and $\Lambda$ is a diagonal matrix that is
...
(\acronym{SVD})\footnote{The Wikipedia pages on PCA, SVD, and other
...
our expression, and we have dropped some constants\footnote{The
  ...
  assuming (see Assumptions above) that we know all four tensors $C_1$,
  $C_2$, $V$, and $W$. If that \emph{isn't} true, and this analysis
  were to be expanded to one in which any or all of those variance
  tensors was to be learned or inferred or marginalized out, these
  log-determinant terms could not be ignored. Just a word to the
  wise.} and used some more matrix-algebra foo.
...
Forth, the operation $V\cdot\inv{C}$ is effectively taking the ratio
...
in this document, and also retain my day job. Some people think about
...
linear algebra.\footnote{Even though there hasn't been nearly as much
  mathematical mess written in this document as there has been written
...
forward or save a factorization.\footnote{Another piece of advice, but
  much less relevant to the present document, is to use \code{logdet}
  or \code{slogdet} not \code{det} when you need determinants.}
