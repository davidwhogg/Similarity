...
identical if we had better data?\footnote{I found myself saying,
  ...
  operationally satisfied. Are two stars \emph{truly} identical?  I
...
I hereby define the scope of this \documentname\ such that these
questions are out of scope!

Okay, that's not fair. While leaving a full treatment outside the
scope, here are some comments of possible relevance: FIRST: USE V=INFINITY IF RELEVANT.

SECOND: USE PCA OR EQUIVALENT IF $V\gg C$

THIRD: FIND $W$ BY REGRESSION OR SVM OR SOMETHING LIKE THAT IF DATA ARE GOOD.

FOURTH: BUILD A REAL MODEL IF NONE OF THE ABOVE. SEE HMF AND XD PAPERS.

\paragraph{Relaxing the assumptions:}
HOGG: LET'S GO. TRACTABILITY. APPROACHES.

MIXTURES OF GAUSSIANS.

OTHER PARAMETERS (like true distance, as in Oh \etal, 2017).
