...
% # To-Do:
% - put in proper citations
% - fix typesetting and content of bibliography
% - make more discursive introduction
...
between the observations. I discuss the results in the context of some
astrophysics problems---stellar spectroscopy (stellar twins) and
chemical-abundance-space measurements (chemical tagging)---but it
...
\paragraph{Priors and intrinsic scatter:}
For the benefit of the astrophysicists---or those who have been dying
of suspense---I now return to the original motivating problems to make
a few comments about the scientific implementation of these
methods. While astrophysicists are very good at delivering estimates
...
you don't care about or want to ignore.  How might we
determine\footnote{Recall that we must determine these in order to use
  the method and results in this \documentname: It is literally part
  of the assumptions that we have acceptable estimates of $V$ and
  $W$.} these tensors?
...
First, if your observations are precise, and the population is
broad in most or all dimensions, then just take the limit of large $V$
and $\chi^2$ (though with variance $C\equiv C_1 + C_2 + W$). This is
very often the case when the data are low-dimensional ($D$ is a few).
...
\quad .
\end{eqnarray}
This has the disadvantage that it gives an approximation to the
empirical variance, which has both true-scatter and
observational-scatter contributions. But this approach is often
...
Third, if this latter issue---that the observational scatter is
biasing your $V$ estimate high---is a problem, you might want to do
some kind of real inference to get $V$. That is, if components of $C$
are as large as or larger than relevant components of $V$, you might
want to build a model to infer $V$. We have written up examples of
this (HOGG CITE XD, HMF); one of these examples could be applied to the
chemical-twin case.
...
with eigenvectors that projects strongly onto the eigenbasis of $V$,
there won't be much discriminative power (the product $V\cdot\inv{C}$
will become small). So ideally $W$ is low in rank. There is no magic
...
chemical-abundance space. In this case, empirical differences among
stars that are known to have common origin can be inspected or
\acronym{PCA}-ed or modeled statistically to develop an estimate for
$W$.
...
Both observational uncertainties, the prior, and the nuisance
(intrinsic scatter). This assumption is required for any fast
method. There are other options that might be convex, and there are
...
to take into account, depending on your goals. It is usually worth
